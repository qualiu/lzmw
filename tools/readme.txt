...
Match/Search/Replace String/Lines/Blocks in Command/Files/Pipe. (IGNORE case of file and directory name) by LQM:
  ...
  -K [ --backup ]             Backup files if replaced files content (Rename them by appending last write times like: --bak-2020-11-20__20_47_20).
...
(5) Execute each output line as a command: If has -X (--execute-out-lines): 
    echo Complex symbols command contains 2 types of quotes + slashes, hard to quote. | lzmw -X -M || exit -1
    lzmw -XM -z "Simple command, or you want to keep quotes and other chars." || exit -1
...
Example-7: Display current modified code files:
    for /f %a in ('lzmw -l -f "\.(cs|java|cpp|cx*|hp*|py|scala)$" -rp "%CD%" --nd "^(debug|release)$"  --w1 "2020-11-20 20:47:20" -PAC 2^>nul ') do @echo code file: %a
...
Example-9: Get precise time of now and set to %TimeNow-XXX% variable for latter commands: Now time = 2020-11-20 20:47:20.995074 +0800 CST = China Standard Time
...
(C) Replace with Many filters + Jump out: lzmw -r -p folder1,fileN -w path-lines-1.txt,list-3.txt -k 33 -f "\.(cs|cp*|hp*|cx*)$" --nf "test|unit" -d "^(code|src)$" --nd "^(\.git|Debug)$" --pp "code.*src" --np "bin\S*Release" --xp "bin\Release,obj\,test" -G --xd --xf --w1 2016-02 --w2 2016-02-01T23:30:01 --s1 1B --s2 1.5MB -i -x public -t "\bclass\b" -o Class -e color-extra-regex -U 3 -D 3 -b begin-line-or-block -Q block-end-regex -q stop-regex -L 10 -N 3000 -H 100 -J -m -u -v dtm -O -c Show command + Out summary only if found.
