...
Match/Search/Replace String/Lines/Blocks in Command/Files/Pipe. (IGNORE case of file and directory name) by LQM:
  ...
  -K [ --backup ]             Backup files if replaced files content (Rename them by appending last write times like: --bak-2020-11-29__13_54_10).
...
Example-7: Display current modified code files:
    for /f %a in ('lzmw -l -f "\.(cs|java|cpp|cx*|hp*|py|scala)$" -rp "%CD%" --nd "^(debug|release)$"  --w1 "2020-11-29 13:54:10" -PAC 2^>nul ') do @echo code file: %a
...
Example-9: Get precise time of now and set to %TimeNow-XXX% variable for latter commands: Now time = 2020-11-29 13:54:10.861038 +0800 CST = China Standard Time
