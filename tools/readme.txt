...
Get difference-set(not-in-latter) for first file/pipe; Or intersection-set with latter file/pipe. by LQM:
  ...
  -e [ --enhance ] arg         Regex pattern to enhance text, inferior to: -t -x, used merged Regex value of "(-t)|-e" to enhance if used both -t and -e.
...
(2) Search in files with Regex pattern  : lzmw -rp dir1,dir2,fileN -t "my.*(capture-1).*pattern" -P -I -C
(3) Search files & Replace matched text : lzmw -rp dir1,dir2,fileN -t "my.*(capture-1).*pattern" -o "captured \1 and you want" -P -A -C
