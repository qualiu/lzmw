...
Match/Search/Replace String/Lines/Blocks in Command/Files/Pipe. (IGNORE case of file and directory name) by LQM:
  ...
  -K [ --backup ]             Backup files if replaced files content (Rename them by appending last write times like: --lz-backup-2018-06-08__09_18_30).
...
Example-7 : Display current modified code files:
    for /f %a in ('lzmw -l -f "\.(cs|java|cpp|cx*|hp*|py|scala)$" -rp "%CD%" --nd "^(debug|release)$"  --w1 "2018-06-08 09:18:30" -PAC 2^>nul ') do @echo code file : %a
...
(11) Get precise time of now and set to %TimeNow-XXX% variable for latter commands: Now time = 2018-06-08 09:18:30.240144 +0800 CST = China Standard Time
