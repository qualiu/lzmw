...
Match/Search/Replace String/Lines/Blocks in Command/Files/Pipe. (IGNORE case of file and directory name) by LQM:
  ...
  -K [ --backup ]             Backup files if replaced files content (Rename them by appending last write times like: --lz-backup-2018-05-20__20_41_13).
...
(4) Execute output line as command : If has -X (--execute-out-lines): 
    ...
    -Y to force reading from files other than pipe. -Y is rarely used, only for cases which lzmw cannot know where to read from : files or pipe. For example, remove the -p . and -Y below:
...
Example-7 : Display current modified code files:
    for /f %a in ('lzmw -l -f "\.(cs|java|cpp|cx*|hp*|py|scala)$" -rp "%CD%" --nd "^(debug|release)$"  --w1 "2018-05-20 20:41:13" -PAC 2^>nul ') do @echo code file : %a
...
(3) Find top 100 largest old garbage log files which size >= 30MB:
    lzmw -rp directory-1,dir-2 -f "\.(log)$" -l --sz --s1 30MB --w2 "2015-07-27 12:30" -H 100 --dsc -PIC
(4) Get command lines to delete top 100 largest old garbage log files which size >= 30MB:
    lzmw -rp directory-1,dir-2 -f "\.(log)$" -l --sz --s1 30MB --w2 "2015-07-27 12:30" -H 100 --dsc -PAC | lzmw -t .+ -o "del /f /q /s \"$0\"" -PIC
...
(6) Delete top 100 largest old garbage log files which size >= 30MB:
    lzmw -rp directory-1,dir-2 -f "\.(log)$" -l --sz --s1 30MB --w2 "2015-07-27 12:30" -T 100 -PAC | lzmw -t .+ -o "del /f /q /s \"$0\"" -H 100 -XPI
...
(11) Get precise time of now and set to %TimeNow-XXX% variable for latter commands: Now time = 2018-05-20 20:41:13.469266 +0800 CST = China Standard Time
    ...
    for /f "tokens=*" %a in ('lzmw -hC ^| lzmw -t ".*Now time = (\d+\S+) (\d+[:\d]+)\.(\d{3})(\d*)\s+([-\+]\d+)?\s*(\w+)?.*" -o "\1__\2.\3\4" -PAC ^| lzmw -t ":" -o _ -aPAC') do SET "TimeNowForFileName=%a"
...
(1) Search files by plain text matching : lzmw -rp dir1,dir2,fileN -x "my plain text" -PAC
(2) Search in files with Regex pattern  : lzmw -rp dir1,dir2,fileN -t "my.*(capture-1).*pattern" 
(3) Search files & Replace matched text : lzmw -rp dir1,dir2,fileN -t "my.*(capture-1).*pattern" -o "captured $1 and you want" 
