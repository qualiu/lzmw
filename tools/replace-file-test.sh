...
function Show_Json_Error() {
    ...
    echo Full retry: pushd $ThisDir AND cp -ap sample-block.json sample-block-test.json AND $lzmw $@ AND $nin sample-block-expected-result.json sample-block-test.json | $lzmw -aPA -x AND -o ";" -t "Full retry:" -e "(pushd.+)"
    exit -1
}
