...
lzmw -z "LostArg%~1" -t "^LostArg(|-h|--help|/\?)$" > nul || (
    ...
    echo Example : %0   0    -- directly test and output on this command window, using lzmw.
    echo Example : %0   1    -- output to files and compare with base-windows-**.log, using lzmw.
    echo Example : %0   2    -- output to files and compare with base-windows-**.log, using lzmw, do not break if one test group failed.
    echo Example : %0   0    lzmw       3    -- output to window, sleep 3 seconds for each execution.
    echo Example : %0   0    lzmw-dbg   3
    echo Example : %0   1    lzmw-dbg   0  4
    echo Example : %0   1    lzmw-dbg   0  4  4
    exit /b 0
)
...
for /f "tokens=*" %%a in ('lzmw -hC ^| lzmw -t ".*Now time = (\d+\S+) (\d+[:\d]+)\.(\d{3})(\d*)\s+([-\+]\d+)?\s*(\w+)?.*" -o "\1__\2.\3\4" -PAC ^| lzmw -t ":" -o _ -aPAC') do SET "TimeNowMicro=%%a"

set MSR_UNIFY_NAME=lzmw
...
if %TestGroupNumber% GEQ %BeginTestGroup% if %TestGroupNumber% LEQ %EndTestGroup% (
    ...
    %MSR_EXE% -p %ThisDir%\test-path-dots.bat -x lzmw -o %MSR_EXE% -PAC | %MSR_EXE% -x "%%~dp0" -o "%ThisDir%\\" -aPICc | lzmw -X -I -M > %ThisDir%\tmp-test-path-dots.log
    popd
...
if %TestGroupNumber% GEQ %BeginTestGroup% if %TestGroupNumber% LEQ %EndTestGroup% (
    %MSR_EXE% -p %ThisDir%\color-group-test.cmd -x lzmw -o %MSR_EXE% -aPAC > "%ThisDir%\tmp-color-group-test-%TimeNowMicro%.cmd"
...
lzmw -z "call %ThisDir%\test-nin.cmd 1" -XM
exit /b !ERRORLEVEL!
