...
if "%IsShowUsage%" == "1" (
    ...
    echo Example : %0   0    lzmw-dbg   3    -cA
    exit /b 0
)
...
echo %MSR_EXE% -p example-commands.bat -t "^lzmw -c (.+\s+-o\s+)" --nt "\s+-R" -o "lzmw -c -j $1" -PAC ^| %MSR_EXE% -x %%~dp0 -o . -a -X ^> %onlyOuputReplacedTestLog%
%MSR_EXE% -p example-commands.bat -t "^lzmw -c (.+\s+-o\s+)" --nt "\s+-R" -o "lzmw -c -j $1" -PAC | %MSR_EXE% -x %%~dp0 -o . -a -X > %onlyOuputReplacedTestLog%
...
:Unify_TestLog_Before_Compare
    ...
    %MSR_EXE% -p %1 -b "^%MSR_EXE%.*\s+-l -f bat\s+" -Q "^Matched|^\s*$" -t "\S+\.bat$" -o "Replace-Real-File-Name-To-Avoid-Difference" -ROc
