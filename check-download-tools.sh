...
check_tool() {
    toolName=$1
    # toolPath=$(whereis $toolName 2>/dev/null | lzmw -t "^.*?:\s*(\S+?${toolName}\S*).*" -o '$1' -PAC 2>/dev/null | tr -d '\r')
    ...
    if [ ! -f $ThisDir/$toolName$toolExtension ]; then
        wget "https://github.com/qualiu/lzmw/blob/master/tools/$toolName$toolExtension?raw=true" -O $toolPath.tmp \
    ...
    if [ "$toolName" == "lzmw" ]; then
        export lzmw=$toolPath
        alias lzmw=$toolPath
    else
        export nin=$toolPath
        alias nin=$toolPath
