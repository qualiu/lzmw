...
:: lzmw -p full-Windows-comparison.txt -it ".*Used (\d+\.\d{1,3}).* Read (\w+)\s*:\s*(.+)"  -o "| $2 | $3 | $1 s |" -a -PAC | lzmw -t "Test file.*" -o "$0\n\n| exe | text pattern | cost |\n| -- | -- | -- |" -aPAC
...
lzmw -p %PerformanceFile% -it ".*Used (\d+\.\d{1,3}).* Read (\w+)\s*:\s*(.+)"  -o "| $2 | $1 s | ```$3``` | type-replace | case-replace | rows-replace | size-replace | system-replace |" -a -PAC | lzmw -t "Test file.*" -o "$0\n\n| EXE | Cost | To find | Type | Text case | File rows | File size | System Info |\n| -- | -- | -- | -- | -- | -- | -- | -- |" -aPAC > "%TmpOutput%"

for /f "tokens=*" %%a in ('lzmw -p %PerformanceFile% -it ".*from\s+(\d+\S+ \d+:\d+:\d+)\S*\s*(\w*).*" -o "$1 $2" -PAC -H 1') do set "BeginTestTime=%%a"

for /f "tokens=*" %%a in ('nin "%TmpOutput%" nul ".*:(.+)$" -upd -H 1 -PAC') do set FullSystemInfo=%%a
for /f "tokens=*" %%a in ('lzmw -z "%FullSystemInfo%" -it ".*?((?:%AllSystems%)\S*)\s+(\S+).*" -o "$1 $2" -PAC') do set SystemInfo=%%a
for /f "tokens=*" %%a in ('lzmw -z "%SystemInfo%" -it "[^\w\.\s-]\S*" -o "" -aPAC ^| lzmw -t "\s+" -o " " -aPAC') do set SystemInfo=%%a
for /f "tokens=*" %%a in ('lzmw -z "%SystemInfo%" -it ".*((?:%AllSystems%)\w*).*" -o "$1" -aPAC') do set SystemInfo=%%a

for /f "tokens=*" %%a in ('lzmw -p "%TmpOutput%" -t "Test file info.*?(\d+)\s*lines (\d+\.?\d*\s*\wB).*" -o "$1" -H 1 -PAC') do set "fileRows=%%a"
for /f "tokens=*" %%a in ('lzmw -p "%TmpOutput%" -t "Test file info.*?(\d+)\s*lines (\d+\.?\d*\s*\wB).*" -o "$2" -H 1 -PAC') do set "fileSize=%%a"

lzmw -p "%TmpOutput%" -it rows-replace -o "%fileRows%" -R -c
lzmw -p "%TmpOutput%" -it size-replace -o "%fileSize%" -R -c
lzmw -p "%TmpOutput%" -it system-replace -o "%SystemInfo%" -R -c

set beginPattern=
set endPattern=
set /a tables=0

set blockFinding=lzmw -p "%TmpOutput%" --nt "^(Test file|\s*\|)" -t "^(.*?\d+:\d+:\d+[\w\.:/\s-]+).*" -o "$1" -PIC -c Finding blocks
for /f "tokens=*" %%a in ('%blockFinding%') do (
    ...
    if !tables! EQU 1 (
        ...
        set endPattern=%%a
        set CommonCommand=lzmw -p "%TmpOutput%" -b "!beginPattern!" -q "!endPattern!" --nt "!endPattern!"
...
set CommonCommand=lzmw -p "%TmpOutput%" -b "!beginPattern!"
:: Replace type of Plain of Regex
!CommonCommand! -it "Regex" -c Check pattern type -PAC >nul
if !ERRORLEVEL! GTR 0 ( set typeReplaceTo=Regex ) else ( set typeReplaceTo=Plain )
...
lzmw -p "%TmpOutput%" -it .+ -o "" --nt "^\s*\|" -R -c Keep only table line
...
del "%TmpOutput%"
lzmw -p "%OutputFile%" -t "(\d+)\.(\d+)(\s*s)" -o "**$1**.*$2*$3" -R -c

echo Please see "%OutputFile%" | lzmw -PA -e .+
